...
#########################################
# Extended filtration on simplex tree #
#########################################
...
class ExtendedSimplexTreeLayer(tf.keras.layers.Layer):
    """
    ...
    def call(self, filtration):
        ...
        indices = _ExtendedSimplexTree(self.simplextree, filtration.numpy(), self.dimensions, self.hcf)
...
def _LowerStarSimplexTree(simplextree, filtration, dimensions, homology_coeff_field):
    ...
    # Compute persistence diagram
    simplextree.compute_persistence(homology_coeff_field=homology_coeff_field,persistence_dim_max=True)
